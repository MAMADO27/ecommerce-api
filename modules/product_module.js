...
const set_image_url = (doc) => {
        if (doc.image_cover) {
        const image_url=`${process.env.BASE_URL}/products/${doc.image}`;
        doc.image = image_url;
    }
    if(doc.images.forEach(images => {
        doc.image_list=[];
        const image_url=`${process.env.BASE_URL}/products/${images}`;
        doc.image_list.push(image_url);
    }));
}
